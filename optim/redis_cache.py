...
# def is_employee_available(employee_id: str, date: date): # not in the unavailable employees cache?
#     if not client.sismember("allocated_employees",f"{str(date)}-{employee_id}"):
#         return True
#     else:
#         return False
# def allocate_employee(employee_id: str, date: date): # put in the unavailable employees cache
#     client.sadd("allocated_employees",f"{str(date)}-{employee_id}")

def is_vehicle_available(vehicle_id: str, date: date): # not in the unavailable vehicles cache?
    if not client.sismember("allocated_vehicles",f"{str(date)}-{vehicle_id}"):
        return True
    else:
        return False

def allocate_vehicle(vehicle_id: str, date: date): # put in the unavailable vehicles cache
...
# for employees
# def cache_unallocated_employees(employee_ids: list): # put in the unallocated employees cache
#     for employee_id in employee_ids:
#         if is_employee_available(employee_id, date):
#             client.sadd("unallocated_employees", f"{employee_id}")

# def get_unallocated_employee(employee_id: str, date: date): # get from the unallocated employees cache
...
def cache_unallocated_vehicles(vehicle_ids: list, date: date):
    for vehicle_id in vehicle_ids:
        if is_vehicle_available(vehicle_id, date): # check allocated_vehicles cache
            client.sadd("unallocated_vehicles", f"{vehicle_id}")

def get_unallocated_vehicle(date: date):
    available_vehicle = client.spop(f"unallocated_vehicles")
    if available_vehicle:
        allocate_vehicle(vehicle_id=available_vehicle, date=date) # allocate to allocated_vehicles cache
    return available_vehicle

async def get_vehicle_from_cache_or_db(app: FastAPI, date: date):
    vehicle_id = get_unallocated_vehicle(date=date)
    if vehicle_id:
        return vehicle_id
    vehicles = await app.mongodb["vehicles"].aggregate([{"$sample": {"size": 100}}, {'$project': {'_id': 1 }}]).to_list(length=None)
    vehicle_ids = [vehicle['_id'] for vehicle in vehicles]
    cache_unallocated_vehicles(vehicle_ids, date)
    await get_vehicle_from_cache_or_db(app=app, date=date)
